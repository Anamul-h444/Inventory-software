...
let API = "http://localhost:5000/api/supplier";
...
export const createUpdateSupplierRequest = async (supplier, objectId) => {
  try {
    ...
    const result = await axios.post(URL, supplier, AxiosHeader);
...
export const supplierListService = async (pageNo, perPage, searchValue) => {
  try {
    store.dispatch(setLoader(true));
    ...
    if (result.status === 200 && result.data.success === true) {
      if (result.data["data"][0]["rows"].length > 0) {
        ...
        store.dispatch(setSupplierListTotal(result.data["data"][0]["total"]));
      } else {
...
export const getSupplierService = async (id) => {
  try {
    ...
    store.dispatch(setLoader(false));
    if (result.status === 200 && result.data.success === true) {
      let FormValue = result.data["supplier"][0];
