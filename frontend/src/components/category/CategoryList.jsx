...
const CategoryList = () => {
  ...
  const totalCategories = useSelector((state) => state.category.ListTotal);

  //Call Api
  useEffect(() => {
    (async () => {
      await categoryListService(pageNo, perPage, searchKey);
    })();
  }, [pageNo]);
  ...
  const handlePerPage = async (e) => {
    ...
    await categoryListService(pageNo, e.target.value, searchKey);
  };
  ...
  const totalData = totalCategories ? totalCategories[0].total : 0;
