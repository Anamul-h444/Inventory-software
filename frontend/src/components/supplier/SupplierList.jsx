...
import {deleteSupplierRequest, supplierListService} from "../../api/ApiSupplier";
...
const SupplierList = () => {
  ...
  const totalSupplier = useSelector((state) => state.supplier.ListTotal);

  //Call Api
  useEffect( () => {
    (async ()=>{await supplierListService(pageNo, perPage, searchKey);})()
  }, [pageNo]);

  //Update state with onChange
  const handlePerPage =async (e) => {
    setPerPage(e.target.value);
    await supplierListService(pageNo, e.target.value, searchKey);
  };
  const handleSearchKey =async (e) => {
    setSearchKey(e.target.value);
    if (e.target.value === 0) {
     await supplierListService(pageNo, perPage, searchKey);
    }
  };
  ...
  const totalData = totalSupplier ? totalSupplier[0].total : 0;
                  ...
                  <tr>
                    ...
                    <td className="flex space-x-3 items-center">
                      <span
                        ...
                        data-tip="Update"
                        onClick={() =>
                          navigate(`/supplier/update/${supplier._id}`)
                        }
                      >
                        <FaRegEdit />
                      </span>
